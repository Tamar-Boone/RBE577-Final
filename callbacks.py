...
class GradientMonitor(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        if batch % self.log_frequency == 0:
            ...
            rmse_log = logs.get('RMSE_log', 0) if logs else 0
            
            # Get some basic info about model state
            trainable_vars = self.model.trainable_variables
            total_params = np.sum([np.prod(v.shape) for v in trainable_vars])
            grad_info = ""
            
            # Check if we have gradients in optimizer
            if hasattr(self.model.optimizer, '_gradients'):
                grads = [g for g in self.model.optimizer._gradients if g is not None]
                if grads:
                    grad_norms = [tf.norm(g).numpy() for g in grads if g is not None]
                    avg_grad_norm = np.mean(grad_norms) if grad_norms else 0
                    grad_info = f", Avg Grad Norm: {avg_grad_norm:.6f}"
            
            print(f"Batch {batch}: Loss = {loss:.4f}, RMSE_log = {rmse_log:.4f}{grad_info}")
