...
class RootMeanSquaredLogError(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        # FIX: Mask out invalid high values (like 65536.0)
        valid_mask = tf.cast(tf.logical_and(tf.greater(y_true, 0.), tf.less(y_true, 2000.0)), tf.float32)
        
        y_true = tf.math.log(y_true + 1e-6)
        y_pred = tf.math.log(y_pred + 1e-6)
        
        error = tf.square(y_true - y_pred)
        # Apply the more robust mask
        error = masked_reduce_mean(error, valid_mask) # Passing mask as 2nd arg works if masked_reduce_mean expects a mask tensor
