...
class DataLoaderGeneric():
    ...
    def _decode_samples(self, data_sample):
        ...
        image_path = data_sample["camera_l"]
        depth_path = tf.strings.regex_replace(image_path, "color_left", "stereo_disparity")
        depth_path = tf.strings.regex_replace(depth_path, ".jpg", ".png")
        
        im_color = self._load_image(image_path)
        im_depth = self._load_image(depth_path, is_depth=True)

        # Read pose information
        rot = tf.stack([data_sample["qw"], data_sample["qx"], data_sample["qy"], data_sample["qz"]], axis=-1)
        pos = tf.stack([data_sample["tx"], data_sample["ty"], data_sample["tz"]], axis=-1)
        ...
        # NEW TRAJ LOGIC: True only for the first frame of each sequence
        # Since we're processing individual frames here, we can't determine sequence boundaries.
        # This will be handled in _build_sequence_samples where we have the full sequence.
        # For now, set all to False - the correct values will be set later.
        new_traj_value = tf.constant(False)
    ...
    def _build_sequence_samples(self, data_sample):
        ...
        self.out_data["RGB_im"] = tf.clip_by_value(self.out_data["RGB_im"], 0.0, 1.0)
        self.out_data["rot"] = rot
        self.out_data["trans"] = pos
        
        # FIXED: TensorFlow-compatible new_traj pattern
        batch_size = tf.shape(im_color)[0]
        seq_len = tf.shape(im_color)[1]
        
        # Create base pattern: [True, False, False, ...] for sequence length
        first_frame_true = tf.constant([True])
        remaining_frames_false = tf.fill([seq_len - 1], False)
        base_pattern = tf.concat([first_frame_true, remaining_frames_false], axis=0)
        
        # Create a matrix where only the first sequence gets the base pattern
        # All other sequences get all False
        first_sequence_mask = tf.range(batch_size) == 0
        first_sequence_mask = tf.reshape(first_sequence_mask, [batch_size, 1])
        
        # Expand base pattern to match batch size
        base_pattern_expanded = tf.tile(tf.reshape(base_pattern, [1, seq_len]), [batch_size, 1])
        
        # Create all-false pattern for other sequences
        all_false_pattern = tf.fill([batch_size, seq_len], False)
        
        # Use tf.where to select base pattern for first sequence, all-false for others
        new_traj_matrix = tf.where(first_sequence_mask, base_pattern_expanded, all_false_pattern)
        
        self.out_data["new_traj"] = new_traj_matrix
