...
class DataLoaderUseGeo(DataLoaderGeneric):
    ...
    def _decode_samples(self, data_sample):
        ...
        if 'disp' in data_sample:
            ...
            # usegeo depth is float32 tiff - read raw bytes and decode
            # tensorflow doesn't have native tiff support, so we use a workaround:
            # read as raw bytes and use tf.image.decode_image with expand_animations=False
            depth_raw = tf.io.read_file(depth_path)

            # decode tiff using opencv through py_function
